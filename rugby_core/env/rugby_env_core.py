...
try:
  import rugby_engine as libgame
  from rugby_engine import GameState
except ImportError:
  print('Cannot import rugby_engine. Package was not installed properly.')
from rugby_core.env import config as cfg
from rugby_core.env import constants
from rugby_core.env import rugby_action_set
from rugby_core.env import observation_processor
import numpy as np
...
class FootballEnvCore(object):
  ...
  def __init__(self, config):
    ...
    self._config = config
    self._sticky_actions = rugby_action_set.get_sticky_actions(config)
  ...
  def reset(self, inc=1):
    ...
    self._action_set = rugby_action_set.get_action_set(self._config)
  ...
  def step(self, action, extra_data={}):
    ...
    action = [
        rugby_action_set.named_action_from_action_set(self._action_set, a)
        for a in action
    ]
    self._step_count += 1
    assert len(action) == (
    ...
    for left_team in [True, False]:
      ...
      for i in range(agents):
        ...
        if self._env.waiting_for_game_count == 20:
          player_action = rugby_action_set.action_short_pass
        elif self._env.waiting_for_game_count > 20:
          player_action = rugby_action_set.action_idle
          ...
          if self._observation['ball_owned_team'] != -1 and self._observation[
            ...
            if bool(self._env.waiting_for_game_count < 30) != bool(left_team):
              player_action = rugby_action_set.action_left
            else:
              player_action = rugby_action_set.action_right
        action_index += 1
        assert isinstance(player_action, rugby_action_set.CoreAction)
