...
from rugby_core.env import config
from rugby_core.env import rugby_action_set
from rugby_core.env import rugby_env
from rugby_core.env import wrappers
from rugby_core.env import scenario_builder
import numpy as np
import psutil
...
class FootballEnvTest(parameterized.TestCase):
  ...
  def test___control_all_players(self):
    ...
    try:
      rugby_core.env.create_environment(
    ...
    env = rugby_core.env.create_environment(
        env_name='tests.multiagent_wrapper',
        rewards='checkpoints,scoring',
    ...
    self.assertIn(obs, env.observation_space)
    obs, _, _, _ = env.step([rugby_action_set.action_left])
    self.assertEqual(np.shape(obs), (72, 96, 4))
    env = rugby_core.env.create_environment(
        env_name='tests.multiagent_wrapper',
        rewards='checkpoints,scoring',
    ...
    obs, _, _, _ = env.step(
        [rugby_action_set.action_idle, rugby_action_set.action_idle])
    obs, _, _, _ = env.step(
        [rugby_action_set.action_left, rugby_action_set.action_right])
    self.assertLen(obs, 2)
  ...
  def test_score_empty_goal(self):
    ...
    last_o = env.reset()[0]
    for _ in range(120):
      o, reward, done, _ = env.step(rugby_action_set.action_right)
      o = o[0]
      if done:
  ...
  def test_second_half(self):
    ...
    env = rugby_env.FootballEnv(cfg)
    for _ in range(5):
      o, _, done, _ = env.step(rugby_action_set.action_idle)
      self.assertFalse(done)
  ...
  def test___render(self):
    ...
    hash_value = observation_hash(o)
    for _ in range(10):
      o, _, _, _ = env.step(rugby_action_set.action_right)
      hash_value = observation_hash(o, hash_value)
    # Linux
  ...
  def test_different_action_formats(self):
    ...
    env = rugby_env.FootballEnv(cfg)
    env.reset()
    env.step(rugby_action_set.action_right)
    env.step([rugby_action_set.action_right])
    env.step(np.array([rugby_action_set.action_right]))
    env.step(np.array(rugby_action_set.action_right))
    env.close()
  ...
  def test_multi_instance(self):
    ...
    def run_episode():
      ...
      env = rugby_env.FootballEnv(cfg)
      actions = len(rugby_action_set.get_action_set(cfg))
  ...
  def test_multi_render(self):
    ...
    if 'UNITTEST_IN_DOCKER' in os.environ:
      ...
      return
    cfg = config.Config({})
    env1 = rugby_env.FootballEnv(cfg)
    env1.render()
    env1.reset()

    env2 = rugby_env.FootballEnv(cfg)
    try:
      env2.render()
    except AssertionError:
      env1.close()
      ...
      env3 = rugby_env.FootballEnv(cfg)
      env3.reset()
      env3.close()
      return
  ...
  def test__memory_usage(self):
    ...
    for _ in range(100):
      _, _, _, _ = env.step(rugby_action_set.action_right)
  ...
  def test_setstate(self, seed):
    ...
    env1 = rugby_env.FootballEnv(cfg1)
    env2 = rugby_env.FootballEnv(cfg2)
    initial_obs = env1.reset()
    env2.reset()
    initial_state = env1.get_state()
    env2.set_state(initial_state)
    random.seed(seed)
    actions = len(rugby_action_set.get_action_set(cfg1))
  ...
  def test_symmetry(self, seed):
    ...
    random.seed(seed)
    action_cnt = len(rugby_action_set.get_action_set(cfg1))
  ...
  def test_offside(self, episode, team2, reverse):
    ...
    env = rugby_env.FootballEnv(cfg)
    env.reset()
    o, _, done, _ = env.step(rugby_action_set.action_long_pass)
    done = False
  ...
  def test_corner(self, episode, factor, reverse):
    ...
    while not done:
      o, _, done, _ = env.step([rugby_action_set.action_left,
  ...
  def test_penalty(self):
    ...
    while not done:
      o, _, done, _ = env.step([rugby_action_set.action_sliding])
  ...
  def test_keeper_ball(self, episode, factor, reverse):
    ...
    while not done:
      o, _, done, _ = env.step([rugby_action_set.action_right,
                                rugby_action_set.action_right])
  ...
  def test_goal(self, episode, reverse):
    ...
    while not done:
      o, _, done, _ = env.step(
          [rugby_action_set.action_right, rugby_action_set.action_right])
  ...
  def test_get_state_wrapper(self):
    ...
    hash1 = 0
    while reward1 < 0.9:
      o, r, _, _ = env.step(rugby_action_set.action_right)
      reward1 += r
