...
from rugby_core.env import observation_preprocessing
from rugby_core.env import player_base
from rugby_core.examples import models  
import joblib
import numpy as np
...
class Player(player_base.PlayerBase):
  ...
  def take_action(self, observation):
    ...
    actions = [rugby_action_set.action_set_dict[self._action_set][action]]
    return actions
...
class DummyEnv(object):
  ...
  def __init__(self, action_set, stacking):
    self.action_space = gym.spaces.Discrete(
        len(rugby_action_set.action_set_dict[action_set]))
