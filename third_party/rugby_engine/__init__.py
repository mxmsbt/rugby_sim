...
rugby_engine_dir = os.path.dirname(os.path.abspath(__file__))
font_file = os.path.join(rugby_engine_dir, 'fonts/AlegreyaSansSC-ExtraBold.ttf')
...
try:
  ...
  if not (os.path.isfile(os.path.join(game_path, 'libgame.so')) and
    ...
    logging.warning('  pushd {} && ln -s libgame.so '
                    '_rugby_engine.so && popd'.format(engine_path))
  raise
