...
class CMakeExtension(Extension):
  ...
  def __init__(self, name):
    ...
    sources = ['third_party/rugby_engine/src/misc/empty.cpp']
...
class CustomBuild(build_ext):
  ...
  def run_unix(self):
    if os.path.exists(self.build_lib):
      dest_dir = os.path.join(self.build_lib, 'rugby_engine')
      os.makedirs(dest_dir, exist_ok=True)
    else:
      # For the development install (pip install -e .)
      # rugby_engine module has to be located in the project root directory.
      dest_dir = "rugby_engine"
      if not os.path.exists(dest_dir):
        try:
          os.symlink(os.path.join(REPO_ROOT, 'third_party/rugby_engine'),
                     dest_dir)
        except:
    ...
    if use_prebuilt_lib:
      prebuilt = os.path.join(
          REPO_ROOT,
          'third_party/rugby_engine/lib/prebuilt_rugby_engine.so')
      if os.system(f'cp "{prebuilt}" "{dest_dir}/_rugby_engine.so"'):
        raise OSError(
      ...
      if os.system(
          f'{build_env} bash "{os.path.join(REPO_ROOT, "rugby_core/build_game_engine.sh")}"'):
      ...
      libs = glob.glob(os.path.join(REPO_ROOT,
                                    'third_party/rugby_engine/_rugby_engine*.so'))
      if not libs:
        raise OSError(
            'RugbySim compilation did not produce _rugby_engine*.so. '
            'Check the CMake output for missing native dependencies.')
      copy_compiled_libs(libs, dest_dir)
    copy_fonts(dest_dir)
    copy_fonts(os.path.join(REPO_ROOT, 'third_party/rugby_engine'))

  def run_windows(self):
    guide_message = 'Please follow the guide on how to install prerequisites: ' \
                    'https://github.com/google-research/football/blob/master/rugby_core/doc/compile_engine.md#windows'
    ...
    if os.path.exists(self.build_lib):
      ...
      if not os.path.exists(dest_dir):
        try:
          os.symlink(os.path.abspath('third_party/rugby_engine'), dest_dir, target_is_directory=True)
        except OSError:
          # Windows doesn't support symlinks for unprivileged users
          # Fall back to copying the files
          os.mkdir(dest_dir)
          shutil.copy2('third_party/rugby_engine/__init__.py', dest_dir)
          data_dir = os.path.join(dest_dir, 'data')
          if not os.path.exists(data_dir):
            shutil.copytree('third_party/rugby_engine/data', data_dir)
    ...
    if os.system('rugby_core\\build_game_engine.bat'):
      raise OSError('Google Research Football compilation failed.\n' + guide_message)
    # Copy compiled library and its dependencies
    lib_path = 'third_party/rugby_engine/build_win/Release/'
...
def process_develop_setup():
  ...
  if 'develop' in sys.argv and os.path.exists('build'):
    ...
    shutil.rmtree('build')
  elif 'develop' not in sys.argv and os.path.exists('rugby_engine'):
    # If `pip install .` is called after development mode,
    # remove the 'fonts' directory copied by a `develop` setup
    copied_fonts = 'third_party/rugby_engine/fonts'
    if os.path.exists(copied_fonts):
      shutil.rmtree(copied_fonts)
    # Remove .so files (.pyd on Windows)
    for empty_lib in glob.glob("rugby_cpp_engine*"):
      os.remove(empty_lib)
    # Finally, remove symlink to the rugby_engine directory
    if not os.path.exists('rugby_engine'):
      return
    if os.path.islink('rugby_engine'):
      if platform.system() == 'Windows':
        os.remove('rugby_engine')
      else:
        os.unlink('rugby_engine')
    else:
      shutil.rmtree('rugby_engine')
...
setup(
    ...
    license='Apache 2.0',
    packages=packages,
    package_dir={'rugby_engine': 'third_party/rugby_engine'},
    ...
    ext_modules=[CMakeExtension('rugby_cpp_engine')],
